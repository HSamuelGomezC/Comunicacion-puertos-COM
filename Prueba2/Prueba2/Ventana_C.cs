...
namespace Prueba2
{
    public partial class Ventana_C : Form
    {

        private SerialPort puertoCOM;
        private bool opcionCActiva = false;
        ...
        private void Ventana_C_Load(object sender, EventArgs e)
        {
            this.FormClosing += Ventana_C_FormClosing;


            // Verificar si la opción 'OpcionA' está activada en la configuración
            opcionCActiva = Settings1.Default.OpcionC;
                ...
                while (true) // Un bucle infinito para continuar intentando hasta recibir "K"
                 {
                    ...
                    else
                    {
                        ...
                        while (opcionCActiva && !detenerHiloLectura)
                        {
                            // Enviar el comando 'F#08' a la báscula
                            EnviarComando("F#08");

                            // Enviar el comando 'F#07' a la báscula
                             EnviarComando("F#07");

                             string cadena = LeerRespuestaBascula();

                            if (cadena.EndsWith("R3X"))
                            {
        ...
        private void Ventana_C_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void btnEnviar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "INSERT INTO dbo.TablaC (Arena,Grava1,Grava2,Grava3,Cemento,Agua,Humedad,Hora,Fecha,Lote) " +
        ...
        private void btnFecha_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "SELECT * FROM dbo.TablaC WHERE Fecha = @Fecha";
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
        ...
        private void btnLote_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "SELECT * FROM dbo.TablaC WHERE Lote = @Lote";
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "SELECT * FROM dbo.TablaC";
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
        ...
        private void ExportarDataGridViewAExcel(DataGridView dataGridView, string filePath)
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                var worksheet = package.Workbook.Worksheets.Add("Datos Nave C");
