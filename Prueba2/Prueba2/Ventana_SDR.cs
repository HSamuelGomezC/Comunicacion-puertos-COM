...
namespace Prueba2
{
    public partial class Ventana_SDR : Form
    {
        private SerialPort puertoCOM;
        private bool opcionSDRActiva = false;
        ...
        private void Ventana_SDR_Load(object sender, EventArgs e)
        {
            this.FormClosing += Ventana_SDR_FormClosing;


            // Verificar si la opción 'OpcionA' está activada en la configuración
            opcionSDRActiva = Settings1.Default.OpcionSDR;
            ...
            if (opcionSDRActiva)
            {
                ...
                while (true) // Un bucle infinito para continuar intentando hasta recibir "K"
                {
                    ...
                    else
                    {
                        ...
                        while (opcionSDRActiva && !detenerHiloLectura)
                        {
        ...
        private void Ventana_SDR_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            foreach (char letra in letraIndices.Keys)
            {
                ...
                switch (letra)
                {
                    ...
                    case 'F':
                        ...
                        break;
                    case 'G':
                        textBoxAditivo.Text = valor;
                        break;
                    case 'H':
                        textHRArena1.Text = valor;
                        break;
                    case 'I':
                        textBoxHrGrava3.Text = valor;
                        break;
                    case 'J':
                    ...
                    case 'L':
                        ...
                        break;
                    case 'Z':
                        // Si el valor contiene "R4X", eliminarlo
                        if (valor.Contains("R4X"))
                        {
                            valor = valor.Replace("R4X", string.Empty);
                        }
                        textBoxHRC.Text = valor;
                        break;
                }
            }
        ...
        private void btnEnviar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string aditivo = textBoxAditivo.Text;
                string hrArena = textHRArena1.Text;
                string hrGrava = textBoxHrGrava3.Text;
                string hora = textHora1.Text;
                string fecha = textFecha1.Text.Replace(" ", "");
                string lote = textLote.Text;
                string HRC = textBoxHRC.Text;
                ...
                string query = "INSERT INTO dbo.TablaSDR (Arena,Grava1,Grava2,Grava3,Cemento,Agua,Aditivo,HRArena,HRGrava,Hora,Fecha,Lote,HRC) " +
                               "VALUES (@Arena, @Grava, @Grava2, @Grava3, @Cemento, @Agua, @Aditivo, @HRArena, @HRGrava, @Hora, @Fecha, @Lote, @HRC)";
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@Aditivo", aditivo);
                        command.Parameters.AddWithValue("@HRArena", hrArena);
                        command.Parameters.AddWithValue("@HRGrava", hrGrava);
                        command.Parameters.AddWithValue("@Hora", hora);
                        command.Parameters.AddWithValue("@Fecha", fecha);
                        command.Parameters.AddWithValue("@Lote", lote);
                        command.Parameters.AddWithValue("@HRC", HRC);
        ...
        private void btnFecha_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "SELECT * FROM dbo.TablaSDR WHERE Fecha = @Fecha";
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
        ...
        private void btnLote_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "SELECT * FROM dbo.TablaSDR WHERE Lote = @Lote";
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "SELECT * FROM dbo.TablaSDR";
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
        ...
        private void ExportarDataGridViewAExcel(DataGridView dataGridView, string filePath)
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                var worksheet = package.Workbook.Worksheets.Add("Datos Nave SDR");
